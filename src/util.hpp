...
constexpr std::size_t operator"" _st(unsigned long long number) //-V2575
{
    return static_cast<std::size_t>(number);
}

constexpr std::ptrdiff_t operator"" _pd(unsigned long long number) //-V2575
{
...
namespace util
{
    ...
    [[nodiscard]] inline float angleFromVector(const sf::Vector2f & velocity)
    {
        ...
        const float angleRadians{ acosf(posDiffNormal.x) };
    ...
    struct Stats
    {
        ...
        {
            std::ostringstream ss;
            ss.imbue(std::locale("")); // this is only to put commas in the big numbers //-V2578

            ss << "x" << count << " [" << min << ", " << static_cast<T>(avg) << ", " << max
               << "] sd=" << sdv;
