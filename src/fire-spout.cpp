...
namespace halloween
{
    ...
    void FireSpouts::add(const Context & t_context, const sf::FloatRect & t_region)
    {
        ...
        spout.spout_sprite.setPosition(
            { (util::center(t_region).x - (spoutBounds.size.x * 0.5f)),
              (util::bottom(t_region) - spoutBounds.size.y) });
        ...
        spout.fire_sprite.setPosition(
            { (util::center(spoutBounds).x - (fireBounds.size.x * 0.5f)),
        ...
        spout.time_between_spurts_sec =
            t_context.random.fromTo(m_timeBetweenSpurtsMinSec, m_timeBetweenSpurtsMaxSec);
    }
    ...
    void FireSpouts::update(const Context &, const float t_frameTimeSec)
    {
        for (FireSpout & spout : m_fireSpouts)
        {
            ...
            {
                spout.elapsed_time_sec += t_frameTimeSec;
    ...
    void FireSpouts::moveWithMap(const sf::Vector2f & t_move)
    {
        for (FireSpout & spout : m_fireSpouts)
        {
            spout.spout_sprite.move(t_move);
            spout.fire_sprite.move(t_move);
        }
    }

    bool FireSpouts::doesAvatarCollideWithAnyAndDie(const sf::FloatRect & t_avatarRect) const
    {
        for (const FireSpout & spout : m_fireSpouts)
        {
            if (!spout.is_spurting)
            ...
            if (t_avatarRect.findIntersection(spoutRect))
            {
                return true;
            }
        }
    ...
    void FireSpouts::appendCollisions(std::vector<sf::FloatRect> & t_rects) const
    {
        for (const FireSpout & spout : m_fireSpouts)
        {
            t_rects.push_back(spout.spout_sprite.getGlobalBounds());
        }
    }
